...
def generate_stratified_splits_for_subjects(num_subjects, n_splits=5, test_size=0.2):
    ...
    for subject_id in range(1, num_subjects + 1):
        ...
        sss = StratifiedShuffleSplit(
            n_splits=n_splits, test_size=test_size, random_state=None
        )
...
def create_dynamic_dataset(
    subject_ids, num_classes, fold_idx, stratified_indices_dict, is_train
):
    datasets = []
    for subject_id in subject_ids:
        ...
        gen_func = (
            lambda subject_id=subject_id, is_train=is_train, num_classes=num_classes, stratified_indices=indices: (
                load_data_on_demand(subject_id, num_classes, stratified_indices)
            )
        )
...
def train_and_evaluate(
    ...
    if os.path.exists(model_save_path) and overwrite == False:
        ...
        log_dir = (
            f"./paper_logs/category_classifier/model_fold_{fold_idx + 1}_"
...
def main():
    ...
    subject_splits = generate_stratified_splits_for_subjects(
    ...
    for fold_idx in range(num_splits):  # For each fold
        ...
        if history is not None:
            ...
            # Save the figure
            plt.savefig(
                f"{figures_dir}/category_classification_fold_{fold_idx + 1}_metrics.png"
            )
            plt.close()
