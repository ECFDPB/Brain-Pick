...
def k_fold_train_subject_model(subject, data, labels, k=5):
    ...
    fig_subject_dir = (
        f"figures/likeness_degree_individual_users_{k}_folds/Subject_{subject}"
    )
    models_subject_dir = (
        f"paper_models/likeness_degree_individual_users_{k}_folds/Subject_{subject}"
    )
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(data, labels)):
        ...
        log_dir = (
            f"./paper_logs/identity_classifier/model_fold_{fold + 1}_"
        ...
        callbacks = [
            ...
            ModelCheckpoint(
                f"{models_subject_dir}/S{subject}_fold_{fold + 1}.h5",
                save_best_only=True,
                verbose=1,
            ),
...
def save_results_to_csv(subject_results, k):
    ...
    with open(filepath, "w", newline="") as csvfile:
        ...
        for subject, accuracies in subject_results.items():
            ...
            print(
                f"Subject {subject}: Mean Accuracy = {mean_accuracy:.4f}, Std = {std_accuracy:.4f}"
            )
