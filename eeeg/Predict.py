...
def create_tags_report_from_predictions(
    predicted_classes, predicted_probas, username_prefix="user", start_timestamp=None
):
    """
    ...
    for i, (pred_class, pred_proba) in enumerate(
        zip(predicted_classes, predicted_probas)
    ):
        # 生成用户名
        ...
        # 创建TagsReport对象
        report = TagsReport(username=username, timestamp=timestamp, value=float(value))
...
print(
    f"  value范围: [{tags_report_df['value'].min():.4f}, {tags_report_df['value'].max():.4f}]"
)
...
neutral = tags_report_df[
    (tags_report_df["value"] >= -0.33) & (tags_report_df["value"] <= 0.33)
].shape[0]
