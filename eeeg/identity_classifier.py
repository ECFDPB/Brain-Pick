...
def load_data_on_demand(subject_id, is_train, fold_idx, n_splits, num_classes):
    ...
    val_end_idx = (
        val_start_idx + val_split_size if (fold_idx < n_splits - 1) else total_size
    )

    if is_train:
        # For training, use all data except the validation split
        indices = itertools.chain(
            range(0, val_start_idx), range(val_end_idx, total_size)
        )
    else:
...
def create_dynamic_dataset(subject_ids, num_classes, fold_idx, n_splits, is_train):
    ...
    for subject_id in subject_ids:
        ...
        gen_func = lambda subject_id=subject_id: load_data_on_demand(
            subject_id, is_train, fold_idx, n_splits, num_classes
        )
...
def train_and_evaluate(
    ...
    if os.path.exists(model_save_path) and overwrite == False:
        ...
        log_dir = (
            f"./paper_logs/identity_classifier/model_fold_{fold_idx + 1}_"
        ...
        history = model.fit(
            train_ds, epochs=500, validation_data=val_ds, callbacks=callbacks, verbose=1
        )

    # Directly evaluate the model on the validation dataset
    val_loss, val_accuracy_direct = model.evaluate(val_ds, verbose=0)
    print(
        f"Direct validation accuracy (post-training): {val_accuracy_direct * 100:.2f}%"
    )
    ...
    plt.savefig(
        os.path.join(figures_dir, f"fold_{fold_idx + 1}_confusion_matrix.png"), dpi=300
    )
    plt.close()
...
def main():
    ...
    for fold_idx in range(num_splits):
        ...
        train_ds = create_dynamic_dataset(
            subject_ids, num_classes, fold_idx, num_splits, is_train=True
        )
        val_ds = create_dynamic_dataset(
            subject_ids, num_classes, fold_idx, num_splits, is_train=False
        )
        ...
        val_accuracy, history, user_accuracies = train_and_evaluate(
            train_ds,
            val_ds,
            fold_idx,
            figures_dir,
    ...
    with open(filepath, "w", newline="") as file:
        ...
        for user_id, accuracies in all_user_accuracies.items():
            if len(accuracies) > 0:  # Ensure there's data to calculate mean/std
                ...
                std_accuracy = np.std(accuracies)
                writer.writerow(
                    [user_id + 1, mean_accuracy, std_accuracy]
                )  # Adjust user_id if necessary
            else:
...
history = model.fit(
    ...
    callbacks=[
        ...
        ModelCheckpoint(save_model_path, save_best_only=True, verbose=1),
    ],
    validation_data=(x_test_concat, y_test_concat),
)
