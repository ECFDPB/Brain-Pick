...
def create_test_dataset(test_subjects, batch_size=256):
    ...
    for subject_id in test_subjects:
        ...
        subject_labels = np.ones(
            (subject_data.shape[0],), dtype=np.int32
        )  # All samples labeled as 1 ("denied")
    ...
    test_labels_tensor = tf.keras.utils.to_categorical(
        test_labels_tensor, num_classes=2
...
def compute_weights_for_classes(subject_ids, allowance_number, is_open_set):
    ...
    if is_open_set == False:
        ...
        labels = np.array(
            [0] * allowance_number + [1] * (total_subjects - allowance_number)
        )
    else:
...
def train_and_evaluate(
    ...
    if is_open_set:
        log_dir = (
            f"./paper_logs/authentication_open_set_{allowance_number}_{allowance_number}_model_fold_{fold_number + 1}_"
        ...
        log_dir = (
            f"./paper_logs/authentication/authentication_closed_set_{allowance_number}_{24 - allowance_number}_model_fold_{fold_number + 1}"
            + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        )
...
def save_metrics_summary_to_csv(
    accuracies,
    precisions,
    recalls,
    f1_scores,
    ...
    with open(csv_path, "w", newline="") as csvfile:
        ...
        metrics = [
            ("Accuracy", accuracies, test_accuracies),
        ...
        for metric_name, train_val_scores, test_scores in metrics:
            ...
            if is_open_set:
                ...
                writer.writerow(
                    [metric_name, f"{train_val_mean:.4f}", f"{train_val_std:.4f}"]
                )


def evaluate_model(
    test_dataset, model, threshold=0.5
):  # Adjust the threshold as needed
    # Predict on the test dataset

    y_pred_probs = model.predict(test_dataset)
    print("predicted probabilities: ", y_pred_probs)
    y_pred_classes = (y_pred_probs[:, 1] >= threshold).astype(
        int
    )  # Assuming class 1 is "denied"
...
def main():
    ...
    for fold_number in range(k):
        ...
        class_weight_dict = compute_weights_for_classes(
            training_subjects, allowance_number, is_open_set
        )
        ...
        train_dataset, val_dataset = create_datasets(
            training_subjects, subject_to_label_map, batch_size
        )
        ...
        model, history, accuracy, precision, recall, f1 = train_and_evaluate(
            train_dataset,
            val_dataset,
            fold_number,
            class_weight_dict,
        ...
        if is_open_set:
            ...
            test_accuracy, test_precision, test_recall, test_f1 = evaluate_model(
                test_dataset, model, threshold=threshold
            )
