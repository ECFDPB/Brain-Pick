...
class TrainingPredictionsProcessor:
    ...
    def process_sample(self, sample_size: int = 10) -> List[TagsReport]:
        ...
        for i, (idx, row) in enumerate(sample_data.iterrows()):
            ...
            print(f"  概率分布: NEGATIVE={row['Prob_NEGATIVE']:.4f}, "
                  f"NEUTRAL={row['Prob_NEUTRAL']:.4f}, "
                  f"POSITIVE={row['Prob_POSITIVE']:.4f}")
            
            timestamp = base_timestamp + i
            tags_report = self.converter.convert_with_probabilities(emotion_result, timestamp)
            tags_reports.append(tags_report)
    ...
    def analyze_results(self, tags_reports: List[TagsReport]):
        ...
        print(f"likeness值范围: [{min(likeness_values):.4f}, {max(likeness_values):.4f}]")
        ...
        print(f"  中度负向 (-0.8 < v <= -0.3): {sum(1 for v in likeness_values if -0.8 < v <= -0.3)}")
        print(f"  轻度负向 (-0.3 < v < 0): {sum(1 for v in likeness_values if -0.3 < v < 0)}")
        print(f"  中立 (v = 0): {sum(1 for v in likeness_values if v == 0)}")
        print(f"  轻度正向 (0 < v < 0.3): {sum(1 for v in likeness_values if 0 < v < 0.3)}")
        print(f"  中度正向 (0.3 <= v < 0.8): {sum(1 for v in likeness_values if 0.3 <= v < 0.8)}")
    ...
    def compare_with_true_labels(self):
        ...
        for idx, row in self.data.iterrows():
            ...
            for emotion_name, prob in probs.items():
                base_value = self.converter.EMOTION_TO_LIKENESS.get(emotion_name.upper(), 0.0)
...
def main():
    ...
    processor = TrainingPredictionsProcessor(
        csv_path='training_set_predictions.csv',
        username='EFBPE'
    )
